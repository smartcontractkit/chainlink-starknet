...
fn insecure_sign(z: u256, e: u256) -> (u256, u256, bool) {
    let z_u512: u512 = z.wide_mul((0x1).into());

    // order of the finite group
    let N: u256 = Secp256k1Impl::get_curve_size().try_into().unwrap();
    let n_u512: u512 = N.wide_mul((0x1).into());
    ...
    let product_u512: u512 = product.wide_mul((0x1).into());
