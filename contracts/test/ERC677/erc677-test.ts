...
describe('ERC677', function () {
    ...
    beforeEach(async () => {
        ...
        await sender.invoke(token, 'permissionedMint', { recipient: sender.starknetContract.address, amount: { high: 0n, low: 1000n } })
        await sender.invoke(token, 'transfer', { recipient: sender.starknetContract.address, amount: { high: 0n, low: 100n } })
    ...
    describe('#transferAndCall(address, uint, bytes)', () => {
        ...
        it('transfers the tokens', async () => {
            ...
            await sender.invoke(token, 'transferAndCall', { to: receiver.address, value: { high: 0n, low: 100n }, selector: 0, data: data })
        ...
        it('transfer succeeds with response', async () => {
            const response = await sender.invoke(token, 'transferAndCall', { to: receiver.address, value: { high: 0n, low: 100n }, selector: 0, data: data })
    ...
    describe('when sending to a contract that is not ERC677 compatible', () => {
        ...
        it('throws an error', async () => {
            try {
                await sender.invoke(token, 'transferAndCall', { to: nonERC677.address, value: { high: 0n, low: 1000n }, selector: 0, data: data })
                expect.fail()
            }
            catch (error: any) {
                let { balance: balance1 } = await token.call('balanceOf', {
