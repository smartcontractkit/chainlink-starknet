    ...
    SimpleWriteAccessController
{
    ...
    uint256 public immutable SELECTOR_STARK_UPDATE_STATUS =
        _selectorStarknet("update_status");
    ...
    event GasConfigSet(
        uint256 gasEstimate,
        address indexed gasPriceL1Feed,
        uint32 gasAdjustment
    );
    ...
    function retry() external checkAccess returns (bool) {
        (, int256 latestAnswer, , , ) = AggregatorV3Interface(s_source)
            .latestRoundData();
    ...
    function approximateGasPrice() public view returns (uint256) {
        (, int256 fastGasPriceInWei, , , ) = AggregatorV3Interface(
            s_gasConfig.gasPriceL1Feed
        ).latestRoundData();
        return
            (uint256(fastGasPriceInWei) * uint256(s_gasConfig.gasAdjustment)) /
            100;
    }
