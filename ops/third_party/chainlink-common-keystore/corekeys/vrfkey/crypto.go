...
func FieldHash(msg []byte) *big.Int {
	rv := secp256k1.MustKeccakHash(string(msg)).Big()
	...
	for rv.Cmp(FieldSize) >= 0 {
		rv = secp256k1.MustKeccakHash(string(common.BigToHash(rv).Bytes())).Big()
	}
	return rv
}
...
func ScalarFromCurvePoints(
	...
	return bm.I().SetBytes(secp256k1.MustKeccakHash(string(msg)).Bytes())
}
