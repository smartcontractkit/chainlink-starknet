...
func blockTagParam(blockID starknetrpc.BlockID) (interface{}, error) {
	switch string(blockID.Tag) {
	case BlockTagPreConfirmed, "latest", "pending":
		return string(blockID.Tag), nil
	case "":
...
func isPreConfirmedBlock(blockID starknetrpc.BlockID) bool {
	return string(blockID.Tag) == BlockTagPreConfirmed
}
...
func (c *Client) eventsAtBlock(ctx context.Context, input starknetrpc.EventsInput) (*starknetrpc.EventChunk, error) {
	fromParam, err := blockTagParam(input.EventFilter.FromBlock)
	if err != nil {
		return nil, err
	}
	toParam, err := blockTagParam(input.EventFilter.ToBlock)
	if err != nil {
		return nil, err
	}
	...
	rawInput := map[string]interface{}{
		"event_filter": map[string]interface{}{
			...
			"to_block":   toParam,
			"address":    input.EventFilter.Address.String(),
			"keys":       input.EventFilter.Keys,
		},
		"result_page_request": map[string]interface{}{
			"chunk_size": input.ResultPageRequest.ChunkSize,
		},
	}
	if input.ResultPageRequest.ContinuationToken != "" {
		rawInput["result_page_request"].(map[string]interface{})["continuation_token"] = input.ResultPageRequest.ContinuationToken
	}
...
func eventsInputUsesPreConfirmed(input starknetrpc.EventsInput) bool {
	return isPreConfirmedBlock(input.EventFilter.FromBlock) || isPreConfirmedBlock(input.EventFilter.ToBlock)
}
