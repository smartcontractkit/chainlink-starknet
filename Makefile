...
.PHONY: lint-go-ops
lint-go-ops:
	cd ./ops && golangci-lint --config ../.golangci.yml --color=always --format checkstyle --out-output golangci-lint-ops-report.xml run

.PHONY: lint-go-relayer
lint-go-relayer:
	cd ./relayer && golangci-lint --config ../.golangci.yml --color=always --format checkstyle --out-output golangci-lint-relayer-report.xml run

.PHONY: lint-go-test
lint-go-test:
	cd ./integration-tests && golangci-lint --config ../.golangci.yml  --color=always --format checkstyle --out-output golangci-lint-integration-tests-report.xml run
