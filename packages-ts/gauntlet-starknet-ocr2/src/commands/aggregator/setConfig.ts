...
const makeContractInput = async (input: UserInput): Promise<ContractInput> => {
  const oracles: Oracle[] = input.signers.map((o, i) => ({ signer: o, transmitter: input.transmitters[i] }))
  return {
    oracles: oracles,
    oracles_len: oracles.length,
