...
const makeContractInput = async ({ previousRoundId, previousAnswer, currentRoundId, currentAnswer }: UserInput): Promise<ContractInput> => {
...
const commandConfig: EVMExecuteCommandConfig<UserInput, ContractInput> = {
    ...
    ux: {
        description: 'Validate the status by sending xDomain L2 tx to Starknet UptimeFeed. Caller must have access to validate.',
        examples: [
