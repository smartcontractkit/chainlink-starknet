...
const commandConfig: EVMExecuteCommandConfig<UserInput, ContractInput> = {
  ...
  action: 'withdraw',
  internalFunction: 'withdraw(uint256,address)',
  ux: {
    description: 'Withdraws funds from L1 bridge to L1 recipient',
    examples: [
      `${CATEGORIES.L1_BRIDGE}:withdraw --network=<NETWORK> --recipient=[L1_RECIPIENT_ADDRESS] --amount=[AMOUNT_IN_LINK] [L1_BRIDGE_PROXY_ADDRESS]`,
    ],
  },
  makeUserInput,
  makeContractInput,
